...
def dlog(msg, src = 'core', level = 0, from = caller)
	$dispatcher.log(Zsf::LOG_DEBUG, src, level, msg, from)
end

def elog(msg, src = 'core', level = 0, from = caller)
	$dispatcher.log(Zsf::LOG_ERROR, src, level, msg, from)
end

def wlog(msg, src = 'core', level = 0, from = caller)
	$dispatcher.log(Zsf::LOG_WARN, src, level, msg, from)
end

def ilog(msg, src = 'core', level = 0, from = caller)
	$dispatcher.log(Zsf::LOG_INFO, src, level, msg, from)
end

def rlog(msg, src = 'core', level = 0, from = caller)
	$dispatcher.log(Zsf::LOG_RAW, src, level, msg, from)
end
...
$dispatcher = Zsf::Logging::LogDispatcher.new