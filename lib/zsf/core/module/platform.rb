...
class Zsf::Module::PlatformList
	attr_accessor :list
	def initialize(*args)
		self.list = [ ]
		args.each { |a|
			if a.kind_of?(String)
				list << Zsf::Module::Platform.find_platform(a)
			elsif a.kind_of?(Range)
				b = Zsf::Module::Platform.find_platform(a.begin)
				e = Zsf::Module::Platform.find_platform(a.end)

				children = Zsf::Module::Platform._find_children(b.superclass)
				r        = (b::Rank .. e::Rank)
				children.each { |c|
...
class Zsf::Module::Platform
	...
	class Windows < Zsf::Module::Platform
		Rank  = 100
		Short = 'win'
		class X86 < Windows
			Rank  = 100
